...
public class Reader {
    ...
    private static String keyOnCard = "BREAKMEIFYOUCAN!";
    ...
    public static boolean readMemory(byte[] target, boolean auth, boolean display) {
        ...
        if (display) {
            history += "\n\nReading finished on " + type + "\n--------------------------------";
            Toast.makeText(MyActivity.outer, type + " tag read", Toast.LENGTH_SHORT).show();
        }
        return true;
    }
    ...
    public static boolean updatePage(byte[] data, int dst, boolean auth) {
        ...
        try {
            ...
            history += "\n" + Dump.hex(response) + " <<\n";
            history += "\n" + "writing finished\n--------------------------------";
//                        Toast.makeText(MyActivity.outer, "Writing finished", Toast.LENGTH_SHORT).show();
            return true;

        } catch (IOException e) {
            history += "\n" + "Writing failed: IOException\nTrying to write on protected pages without successful authentication?\n--------------------------------";
//            Toast.makeText(MyActivity.outer, "Writing failed", Toast.LENGTH_SHORT).show();
    ...
    /**
     * Helper for tools section.
     * Write a new authentication key on the card.
     *
     * @param auth   Whether to authenticate or not when writing. Required if the AUTH0 and AUTH1 settings of the
     *               card forbid writing to the pages containing the authentication key.
     * @param newKey String value of the new key.
     * @return boolean value telling if the key was written correctly
     */
    public static boolean writeStringKey(String newKey, boolean auth) {
        byte[] key = new byte[0];
        boolean check = false;
        if (newKey.regionMatches(0, "0x", 0, 2) && newKey.length() == 18) {
            check = true;
            key = getNumeralKey(newKey);
        ...
        if (check) {
            keyOnCard = newKey;
            history += "\n" + "writing authentication key on card:\n" + newKey.toUpperCase() + "\n";
            return writeKey(key, auth);
        } else return false;
    }

    public static boolean writeKey(byte[] key, boolean auth) {
        try {
            if (auth) {
                authenticate(false);
            }
            ...
            for (int i = 0; i < 4; i++) {
                byte[] cmd_ulwrite = new byte[]{(byte) 0xa2, (byte) 0x00,
                        (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00};
                cmd_ulwrite[1] = (byte) (44 + i);
                System.arraycopy(key, i * 4, cmd_ulwrite, 2, 4);
                history += "\n" + Dump.hex(cmd_ulwrite) + " >>\n";
                byte[] response = nfcA_card.transceive(cmd_ulwrite);
                history += "\n" + Dump.hex(response) + " <<\n";
            }
            authKey = keyOnCard;
            history += "\nSuccessfully wrote authentication key on card\n--------------------------------";
            Toast.makeText(MyActivity.outer, "Successfully wrote authentication key on card", Toast.LENGTH_SHORT).show();
            MyActivity.showConsoleHint();
            return true;
        } catch (IOException e) {
            history += "\n" + "Key write failed: IOException\nTrying to write on protected pages without successful authentication?\n--------------------------------";
            Toast.makeText(MyActivity.outer, "Key write failed", Toast.LENGTH_SHORT).show();
            MyActivity.showConsoleHint();
            return false;
        }
    }
     ...
     *
     * @param page Int value of the protection starting page
     * @param auth boolean value whether to authenticate when setting auth
     */
    public static boolean setAuth0(int page, boolean auth) {
        ...
        try {
            ...
            byte[] response = card.transceive(cmd_ulwrite);
            history += "\n" + Dump.hex(response) + " <<\n";

            history += "\nAuth0 set successfully\n--------------------------------\n";
            Toast.makeText(MyActivity.outer, "Auth0 set successfully", Toast.LENGTH_SHORT).show();
            status = true;
        } catch (IOException e) {
            history += "\nAuth0 set error\n--------------------------------\n";
            Toast.makeText(MyActivity.outer, "Auth0 set error", Toast.LENGTH_SHORT).show();
        }
        MyActivity.showConsoleHint();
        return status;
    }
     ...
     * @param auth   boolean value whether to authenticate when setting auth
     */
    public static boolean setAuth1(boolean noRead, boolean auth) {
        byte[] data = new byte[]{0x00, 0x00, 0x00, 0x00};
        boolean status = false;
        if (noRead) {
            data[0] = (byte) 0x00;
        ...
        try {
            ...
            history += "\nAuth1 set successfully\n--------------------------------\n";
            Toast.makeText(MyActivity.outer, "Auth1 set successfully", Toast.LENGTH_SHORT).show();
            status = true;
        } catch (IOException e) {
            history += "\nAuth1 set error\n--------------------------------\n";
            Toast.makeText(MyActivity.outer, "Auth1 set error", Toast.LENGTH_SHORT).show();
        }
        MyActivity.showConsoleHint();
        return status;
    }
